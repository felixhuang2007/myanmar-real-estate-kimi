...
test_api() {
    ...
    if [ "$method" = "GET" ]; then
        if [ "$need_auth" = "1" ] && [ -n "$TOKEN" ]; then
            RESP=$(curl $curl_opts -H "Authorization: Bearer $TOKEN" "$url")
        else
            RESP=$(curl $curl_opts "$url")
        fi
    else
        local headers="-H 'Content-Type: application/json'"
        [ "$need_auth" = "1" ] && [ -n "$TOKEN" ] && headers="$headers -H \"Authorization: Bearer $TOKEN\""
        RESP=$(eval "curl $curl_opts $headers -X $method -d '$data' '$url'")
    fi
    ...
    if echo "$expected_codes" | grep -q "$HTTP_CODE"; then
        ...
        BODY=$(echo "$RESP" | sed '$d' | head -c 100)
        echo -e "${RED}  ✗ FAIL${NC} | $test_name (HTTP $HTTP_CODE)"
        [ -n "$BODY" ] && echo -e "${RED}       $BODY${NC}"
        FAILED=$((FAILED + 1))
    fi
}
...
# 获取验证码
RESP=$(curl -s -w "\nHTTP_CODE:%{http_code}" -X POST "${BASE_URL}/api/auth/send-verification-code" \
...
if [ "$HTTP" = "200" ]; then
    ...
    PASSED=$((PASSED + 1))
    sleep 1
    VERIFICATION_CODE=$(sudo docker exec myanmar-property-db psql -U myanmar_property -d myanmar_property -t -c "SELECT code FROM sms_verification_codes WHERE phone = '${TEST_PHONE}' AND type = 'login' AND expired_at > NOW() ORDER BY created_at DESC LIMIT 1;" 2>/dev/null | xargs)
...
if [ "$HTTP" = "200" ]; then
    ...
    echo -e "${RED}  ✗ FAIL${NC} | 验证码登录 (HTTP $HTTP)"
    FAILED=$((FAILED + 1))
fi
...
# 使用重置密码API设置密码（因为用户没有旧密码）
if [ -n "$TOKEN" ]; then
    echo "  设置密码..."
    # 先获取一个新的验证码用于重置密码
    curl -s -o /dev/null -X POST "${BASE_URL}/api/auth/send-verification-code" \
        -H "Content-Type: application/json" \
        -d "{\"phone\":\"${TEST_PHONE}\",\"type\":\"reset_password\"}"
    sleep 3
    RESET_CODE=$(sudo docker exec myanmar-property-db psql -U myanmar_property -d myanmar_property -t -c "SELECT code FROM sms_verification_codes WHERE phone = '${TEST_PHONE}' AND type = 'reset_password' AND expired_at > NOW() ORDER BY created_at DESC LIMIT 1;" 2>/dev/null | xargs)
    if [ -n "$RESET_CODE" ]; then
        curl -s -w " (HTTP %{http_code})" -X POST "${BASE_URL}/api/auth/reset-password" \
            -H "Content-Type: application/json" \
            -d "{\"phone\":\"${TEST_PHONE}\",\"code\":\"${RESET_CODE}\",\"new_password\":\"${TEST_PASSWORD}\"}"
        echo ""
        sleep 3
    fi
fi

test_api "TEST-004: 密码登录" "POST" "/api/auth/login-with-password" "{\"phone\":\"+959701234567\",\"password\":\"${TEST_PASSWORD}\",\"device_id\":\"${DEVICE_ID}3\"}" "0" "200"
...
test_api "TEST-008: 实名认证" "POST" "/api/users/me/verification" "{\"real_name\":\"Test\",\"id_card_number\":\"123456\",\"id_card_front\":\"url\",\"id_card_back\":\"url\"}" "1" "200 201 202 403"
...
test_api "TEST-014: 创建房源" "POST" "/api/houses" "{\"title\":\"测试房源\",\"description\":\"这是一个测试房源\",\"price\":50000000,\"price_unit\":\"MMK\",\"area\":100,\"bedrooms\":2,\"bathrooms\":1,\"house_type\":\"condo\",\"transaction_type\":\"sale\",\"city_code\":\"yangon\",\"district_code\":\"downtown\",\"address\":\"仰光市中心\",\"images\":[\"https://example.com/image1.jpg\",\"https://example.com/image2.jpg\",\"https://example.com/image3.jpg\",\"https://example.com/image4.jpg\",\"https://example.com/image5.jpg\"]}" "1" "200 201 403"
...
test_api "TEST-018: 房源列表(经纪人)" "GET" "/api/houses/my?page=1&page_size=20" "" "1" "200 403"
...
test_api "TEST-026: 可预约时间段" "GET" "/api/appointments/slots?agentId=1&date=2026-03-28" "" "1" "200 400"
...
test_api "TEST-035: 预约日历" "GET" "/api/appointments/calendar?month=2026-03" "" "1" "404"
